...
public class DNSLookupService {
    ...
    public void iterativeQuery(DNSQuestion question, InetAddress server) {
        ...
        InetAddress useServer = server;

        while (true) {
            Set<ResourceRecord> setR = individualQueryProcess(question, useServer);
            ...
            for (ResourceRecord r: setR) {
                String hostName = r.getTextResult();
                DNSQuestion qsNS = new DNSQuestion(hostName, RecordType.A, RecordClass.IN);
                List<ResourceRecord> addressNS = cache.getCachedResults(qsNS, true);
                if (!addressNS.isEmpty()) {
                    useServer = addressNS.get(0).getInetResult();
                    find = true;
                    break;
                }
            }
            ...
            if (!find) {
                ResourceRecord findIfNotKnowIp = setR.iterator().next();
                String hostName = findIfNotKnowIp.getTextResult();
                DNSQuestion qs = new DNSQuestion(hostName, RecordType.A, RecordClass.IN);
                try {
                    Collection<ResourceRecord> cr = getResultsFollowingCNames(qs, MAX_INDIRECTION_LEVEL_NS);
                    for (ResourceRecord c : cr) {
                        if (c.getRecordType() == RecordType.A && c.getRecordClass() == RecordClass.IN) {
                            useServer = c.getInetResult();
                        }
                    }

                } catch (CNameIndirectionLimitException e) {
                    e.printStackTrace();
                }
    ...
    protected Set<ResourceRecord> individualQueryProcess(DNSQuestion question, InetAddress server) {
        /* TO BE COMPLETED BY THE STUDENT */
        DNSMessage response;
        Set<ResourceRecord> all = null;

        int repeat = 0;
        DNSMessage queryMessage = buildQuery(question);

        while (repeat < MAX_QUERY_ATTEMPTS && all == null) {
            verbose.printQueryToSend(question, server, queryMessage.getID());
            socket.connect(server, DEFAULT_DNS_PORT);
            DatagramPacket snd = new DatagramPacket(queryMessage.getUsed(), queryMessage.getUsed().length, server, DEFAULT_DNS_PORT);
            ...
            byte[] rev = new byte[DNSMessage.MAX_DNS_MESSAGE_LENGTH];
            DatagramPacket gnd = new DatagramPacket(rev, DNSMessage.MAX_DNS_MESSAGE_LENGTH);
            try {
                socket.receive(gnd);
                byte[] revData = gnd.getData();
                int len = gnd.getLength();
                response = new DNSMessage(revData, len);
                if (queryMessage.getID() == response.getID() && response.getQR()) {
                    all = processResponse(response);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
     ...
     * must be built as an iterative (non-recursive) request for a regular with a single question. When the
